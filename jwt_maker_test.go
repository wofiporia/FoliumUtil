...
func TestJwtMaker(t *testing.T) {
	maker, err := ftoken.NewJwtMaker(frandom.RandomString(32))
...
func TestInvalidJWTTokenAlgNone(t *testing.T) {
	payload, err := ftoken.NewPayload(frandom.RandomString(10), "user", time.Minute)
	require.NoError(t, err)
	...
	payload, err = maker.VerifyToken(token)
	require.Error(t, err)
	require.EqualError(t, err, ftoken.ErrInvalidToken.Error())
	require.Nil(t, payload)
}

func TestJwtWrongKey(t *testing.T) {
	maker1, err := ftoken.NewJwtMaker(frandom.RandomString(32))
	require.NoError(t, err)
	...
	maker2, err := ftoken.NewJwtMaker(frandom.RandomString(32)) // 密钥不同
	require.NoError(t, err)
